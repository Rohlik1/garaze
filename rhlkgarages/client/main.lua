...
RegisterCommand('opengarage', function()
    ...
    if currentZone == 'open' then
        ESX.TriggerServerCallback('rhlk_garags:getVehicles', function(vehicles) 
            if #vehicles > 0 then
                local elements = {}
    ...
    elseif currentZone == 'impound' then
        ESX.TriggerServerCallback('rhlk_garags:getImpoundedVehicles', function(vehicles) 
            if #vehicles > 0 then
                local elements = {}
                for k,v in ipairs(vehicles) do
                    local props = json.decode(v.vehicle)
...
RegisterCommand('savevehicle', function()
    ...
    ESX.TriggerServerCallback('rhlk_garags:saveVehicle', function(success) 
        if success then
...
function SpawnVehicle(props)
    ...
    ESX.Game.SpawnVehicle(props.model, garage.SpawnPosition, garage.SpawnPosition.w, function(vehicle)
        if DoesEntityExist(vehicle) then
            ...
            TriggerServerEvent('rhlk_garags:vehicleTakenOut', props.plate)
        end
    end)
end
...
function RetrieveVehicle(props)
    ...
    ESX.TriggerServerCallback('rhlk_garags:returnVehicle', function(success) 
        if success then
